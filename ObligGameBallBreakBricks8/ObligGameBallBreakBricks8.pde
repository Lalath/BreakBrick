...
void drawBricks()  {
  ...
  float numX = 31;
  float numY = 10;
  float[] rectX = new float[31];
  float[] rectY = new float[31];

  for(int x = 0; x < numX; x++)  {
    for(int y = 0; y < numY; y++)  {
      rectX[x] = (x * brickSize) + 50;
      rectY[y] = (y * brickSize) + 50;
      fill(100,100,255);
...
/*
Fra Marius/modifisert fra chatGPT
void setup() {
  size(400, 400);
  background(255);
  stroke(0);
  ...
  for (float i = x; i <= x + w; i += w/15) {
    line(i, y, i, y + w/15);
    line(i, y + w, i, y + w - w/15);
  }
  
  for (float j = y; j <= y + w; j += w/15) {
    line(x, j, x + w/15, j);
    line(x + w, j, x + w - w/15, j);
  }
}
